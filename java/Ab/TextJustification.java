...
public class TextJustification {
	public List<String> justify(String[] words, int maxWidth) {
		...
		int i, j;	// fit words[i] ~ words[j-1] into a line
		for (i = 0; i < words.length; i = j) {
			...
			int space = 1, extra = 0;	// length of slots, and # slots that need extra space
			if (j != i + 1 && j != words.length) {	// more than one words, and not last line
				space = (maxWidth - len) / (j - i - 1) + 1;	// 1 is the basic space between words
				extra = (maxWidth - len) % (j - i - 1);	// first several slots should have an extra space
			}
			for (int k = i + 1; k < j; k++) {
	...
	public static void main(String[] args) {
		...
		List<String> res = s.justify(input, 16);
		for (String r : res) {
