...
public class Solution {
	public int findRadius(int[] houses, int[] heaters) {
		...
		for (int house : houses) {
			...
			// we can ignore houses on heaters because their nearest distance is 0
			if (index < 0) {	// no heater on the exact position of this house
				index = -(index + 1);	// can also write as "index = ~index;"
				int leftDis = index - 1 >= 0 ? house - heaters[index - 1] : Integer.MAX_VALUE;
