...
public class Solution {
    public int[] maxNumber(int[] nums1, int[] nums2, int k) {
        // idea: Greedy & DP. 2 sub-problems: 1. find the max number of length i within one array;
        // 2. merge the max of length i from nums1 and the max of length k-i from nums2 to get max.
        ...
        for (int i = min; i <= max; i++) {
            ...
            res = isGreater(res, 0, temp, 0) ? res : temp;  // update res if temp is greater
        }
        return res;
    }
    ...
    private int[] getMax(int[] nums, int k) {
        ...
        for (int i = 0, j = 0; j < nums.length; j++) {
            // find a larger digit, then replace to the possible highest pos to get larger
    ...
    private int[] merge(int[] nums1, int[] nums2) {
    	...
    	for (int l = 0; l < len; l++) {    // l is the pointer in res
    ...
    private boolean isGreater(int[] nums1, int i, int[] nums2, int j) {
    	...
    	return i != nums1.length;	// remeber to consider the case of different lengths
    }
}